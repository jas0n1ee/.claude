...
mkdir -p /tmp/claude-swarm/inbox/$SESSION/$WORKER_NAME
echo "$TASK" > /tmp/claude-swarm/inbox/$SESSION/$WORKER_NAME/orchestrator.msg
...
```bash
"$SWARM_REPO_ROOT/swarm/bin/swarm-spawn-claude-worker" "$WORKER_NAME" "$TASK"
```
...
```bash
"$SWARM_REPO_ROOT/swarm/bin/swarm-send" "$WORKER_NAME" "$TASK"
```
...
### 定时或手动唤醒 orchestrator
```bash
"$SWARM_REPO_ROOT/swarm/bin/swarm-ping-orchestrator"
```
...
1. **inbox 文件（可靠）**：`/tmp/claude-swarm/inbox/{session}/{worker}.msg`，文件存在 = 未读，内容是 worker 的最后一条完整消息
2. **tmux 推送通知（best-effort）**：直接发到你的输入框

通常你会先收到 tmux 推送通知，内容如下：
...
```bash
SESSION=$(tmux display-message -p '#S')
rm /tmp/claude-swarm/inbox/$SESSION/orchestrator/worker-alice.msg
```

或：

```bash
"$SWARM_REPO_ROOT/swarm/bin/swarm-ack" worker-alice
```
