...
### 创建 worker
```bash
python3 ~/.claude/swarm/swarm.py spawn --name worker-alice --message "你的任务是：..."
```

### 给已有 worker 发新任务
```bash
python3 ~/.claude/swarm/swarm.py send --name worker-alice --message "新任务描述"
```

### 复用 worker（清空上下文后分配新任务）
```bash
python3 ~/.claude/swarm/swarm.py send --name worker-alice --message "/clear"
...
### 回收 worker（任务完成，不再需要）
```bash
python3 ~/.claude/swarm/swarm.py kill --name worker-alice
```
...
### Ping 自己（被外部唤醒时使用）
```bash
python3 ~/.claude/swarm/swarm.py ping --message "检查消息"
```
...
stop-hook 会捕获到这个信号。当前 `swarm.py` 只把该信号记录为 runtime artifact；实际飞书 Bot / notifier 属于后续 Human-Agent 协调层。注意：
